...
# 3. صفحة سجل الحضور والتقارير (مع الدمج الهندسي التام لصفحة A4 واحدة أفقية بدون فراغات)
  ...
  if log_df.empty or log_df["National_ID"].dropna().empty:
    ...
    # توليد صفوف الجدول الرسمية (بارتفاع مدمج ومحسوب للتحكم التام في الصفحة)
    rows_html = ""
    for idx in range(1, 20):
      if idx <= len(filtered_log):
        ...
        rows_html += f"""
                <tr>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;">{idx}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px; font-weight: bold;">{r_serial}</td>
                    <td style="border: 1px solid #444; padding: 1px 4px; text-align: right; font-size: 11px;">{r_name}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;">{r_code}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;">{r_id}</td>
                    <td style="border: 1px solid #444; padding: 1px 4px; text-align: right; font-size: 11px;">{r_prog}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 10px;">{r_datetime}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center;"></td>
                </tr>
                """
      else:
        rows_html += f"""
                <tr>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;">{idx}</td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;"></td>
                    <td style="border: 1px solid #444; padding: 1px 4px; text-align: right; font-size: 11px;"></td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;"></td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;"></td>
                    <td style="border: 1px solid #444; padding: 1px 4px; text-align: right; font-size: 11px;"></td>
                    <td style="border: 1px solid #444; padding: 1px 2px; text-align: center; font-size: 11px;"></td>
...
<head>
    ...
    <style>
        ...
        @media print {{
            ...
            .sheet {{ width: 100%; height: 100vh; margin: 0; padding: 5mm; }}
        }}
    </style>
</head>
<body>
    ...
    report_popup_btn = f"""
        ...
        <button onclick="openReportWindow()" style="width: 100%; background-color: #0b2246; color: white; padding: 14px 20px; border: none; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; font-family: 'Tahoma', sans-serif;">
            🖨️ فتح وعرض كشف الحضور الرسمي للطباعة (مضمن في ورقة واحدة نهائية)
        </button>
        """
